...
# Hack to import path_utils
newpath = Path(Path(__file__).parent.parent, 'utils')
sys.path.append(str(newpath))
from path_utils import windows_to_unix
...
def fix_makefile(text: str, codePath: Path) -> str:
    ...
    for i, path in enumerate(paths):
        ...
        if i == 0:
            ...
            new_lines.append(f'{codePath}: {windows_to_unix(path)} \\')
        else:
            new_lines.append('\t%s%s' % (windows_to_unix(path), " \\" if i != len(paths) - 1 else ""))

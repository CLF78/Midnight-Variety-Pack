...
XML_GAME_ID = 'RMC'
XML_PATCH_NAME = 'Midnight x Variety Pack'
...
for file in INCLUDE_FILES:
    CFLAGS.append(f'-include {escape_win_path(unix_to_windows(file))}')
...
# - The value can be:
#   - None, if the file does not have to be renamed and the destination matches the source hierarchy
...
UI_ASSETS = {
	...
	'RaceMVP': {
        ...
        Path(UI_ASSETS_DIR, 'game_image', 'ctrl', 'battle_total_point.brctr.json5'): None,
        Path(UI_ASSETS_DIR, 'game_image', 'ctrl', 'lap_number.brctr.json5'): None,
        Path(UI_ASSETS_DIR, 'game_image', 'ctrl', 'map.brctr.json5'): None,
        Path(UI_ASSETS_DIR, 'game_image', 'ctrl', 'point.brctr.json5'): None,
        Path(UI_ASSETS_DIR, 'game_image', 'ctrl', 'position.brctr.json5'): None,
        Path(UI_ASSETS_DIR, 'game_image', 'ctrl', 'position_multi.brctr.json5'): None,
        Path(UI_ASSETS_DIR, 'game_image', 'ctrl', 'score.brctr.json5'): None,
...
writer.rule('cw',
            command=f'$cc -D__CODE_REGION__=$region -D__LOG_LEVEL__={LOG_LEVEL} $cflags -c -o $out_conv -MDfile $out.d $in_conv',
            depfile='$out.d',
...
for locale, cupTextFile in zip(LOCALES, CUP_DATA_TEXT_FILES):
    for file, dests in BMG_DESTINATIONS.items():
        ...
        add_to_set(payloadInputs, change_stem_suffix(baseFile, 'MVP.bmg.json5'))
        add_to_set(payloadInputs, change_stem_suffix(baseFile, f'MVP_{locale}.bmg.json5'))

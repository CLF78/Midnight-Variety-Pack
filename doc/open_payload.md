...
|----------------------|--------------|-------------|:----------:|:--------:|-------------------|
| `0x800095C4`<br>`0x800095E4`<br>`0x800095F4` | Frameskip | Main Frameskip Function | ❌ | N/A | N/A |
...
| `0x800D94F0`<br>`0x800E5980`<br>`0x800E5B14` | NATNEG Fixes | Parse Custom Match Commands | ⚠️ | [dwc_match.cpp](/payload/dwc/dwc_match.cpp)<br>[Natneg.cpp](/payload/wiimmfi/Natneg.cpp) | <ul><li>Hint: these hooks can also be used by mods to support receiving custom commands</li><li>Storing RESV command data is not yet implemented</li><li>Support for CTGP's custom commands will not be implemented</li></ul> |
...
| `0x80535C78` | Accurate Timing | Update Race Frame Counter | ❌ | N/A | N/A |
...
| `0x80562AD0` | Frameskip | Disable Model Drawing while Lagging | ❌ | N/A | That's my interpretation at least |
...
| `0x806579B0` | Various | Main Wiimmfi Network Loop | ❌ | N/A | To be implemented:<ul><li>Report match state changes to the server</li><li>Report suspend matrix to the server</li><li>Execute scheduled kicks</li><li>Report AID/PID matrix to the server</li><li>Report host AID to the server</li><li>Report VS and BT region to the server</li><li>Calculate frame lag</li><li>Report frame lag to the server</li><li>Report SELECT packet information to the server</li><li>Stop NATNEG after 11 seconds have passed since the last match state change (unsure)</li><li>Report first four PIDs to the server (???)</li></ul>Will not be implemented:<ul><li>Auto Reconnect update (feature won't be implemented)</li><li>Item cheat self-reporting (pointless measure)</li><li>Countdown item usage self-reporting (pointless measure)</li></ul> |
...
| `0x8065DF44` | Security Fixes | ITEM Packet Data Validation | ⏭ | N/A | Integrated into RCE Fix |
...
| `0x80760A88` | Bug Fixes | Thwomp Antifreeze | ❌ | N/A | N/A |

...
// kmWrite32(0x8057EF30, 0x2C000001); //changes check from if != 2 to if = 1, so that when in a SMT the
// function keeps going kmWrite32(0x8057EF38, 0x418200A4); //ensure mtSpeedMultiplier gets reset when
// driftState = 1, by sending to where SetKartDriftTiers hooks kmWrite32(0x8057EFB4, 0x48000028); //skips the
// SMT charge check and sends unconditionally to SetKartDriftTiers void SetKartDriftTiers(KartMove& movement)
// {
...
// kmWrite32(0x8058889C, 0x2C000003); //changes check from if != 1 to if = 3, so that when in a MT the
// function keeps going kmWrite32(0x805888A8, 0x418200A4); //if in charge state 2, skip to SetBikeDriftTiers
...
// static void CreatePlayerEffects(EffectsManager& mgr) { //adding the resource here as all other breff have
...
// static void DeleteEffectRes(EffectInfo& mgr) {
//     delete(pulEffects);
//     pulEffects = nullptr;
//     delete(vpEffects);
//     vpEffects = nullptr;
...
void LoadLeftPurpleSparkEffects(ExpPlayerEffects& effects, EGG::Effect** effectArray, u32 firstEffectIndex,
                                u32 lastEffectIndex, const MTX34& playerMat2, const VEC3& wheelPos,
                                bool updateScale) {
    ...
    if (umtCharge >= 250 && type == KartStats::KART) {
        ...
        effects.followFadeEffectsByIdx(effectArray, firstEffectIndex, lastEffectIndex, playerMat2, wheelPos,
                                       updateScale);
    }
    else {
        effects.createAndUpdateEffectsByIdx(effectArray, firstEffectIndex, lastEffectIndex, playerMat2,
                                            wheelPos, updateScale);
    }
}
kmCall(0x80698A94, LoadLeftPurpleSparkEffects);

void LoadRightPurpleSparkEffects(ExpPlayerEffects& effects, EGG::Effect** effectArray, u32 firstEffectIndex,
                                 u32 lastEffectIndex, const MTX34& playerMat2, const VEC3& wheelPos,
                                 bool updateScale) {
    const int type = effects.kartObject->getDriftType();
    const u32 umtCharge = effects.kartObject->pointers.kartMove->umtCharge;
...
void LoadOrangeSparkEffects(ExpPlayerEffects& effects, EGG::Effect** effectArray, u32 firstEffectIndex,
                            u32 lastEffectIndex, const MTX34& playerMat2, const VEC3& wheelPos,
                            bool updateScale) {
    const int type = effects.kartObject->getDriftType();
...
// Fade the sparks
void FadeLeftPurpleSparkEffects(ExpPlayerEffects& effects, EGG::Effect** effectArray, u32 firstEffectIndex,
                                u32 lastEffectIndex, const MTX34& playerMat2, const VEC3& wheelPos,
                                bool updateScale) {
    effects.followFadeEffectsByIdx(effectArray, firstEffectIndex, lastEffectIndex, playerMat2, wheelPos,
                                   updateScale);
...
void FadeRightPurpleSparkEffects(ExpPlayerEffects& effects, EGG::Effect** effectArray, u32 firstEffectIndex,
                                 u32 lastEffectIndex, const MTX34& playerMat2, const VEC3& wheelPos,
                                 bool updateScale) {
    effects.followFadeEffectsByIdx(effectArray, firstEffectIndex, lastEffectIndex, playerMat2, wheelPos,
                                   updateScale);
...
void FadeOrangeSparkEffects(ExpPlayerEffects& effects, EGG::Effect** effectArray, u32 firstEffectIndex,
                            u32 lastEffectIndex, const MTX34& playerMat2, const VEC3& wheelPos,
                            bool updateScale) {
    effects.followFadeEffectsByIdx(effectArray, firstEffectIndex, lastEffectIndex, playerMat2, wheelPos,
                                   updateScale);
...
s16 PatchDriftStateCheck(const KartObject& kartObject) {
    ...
    if (driftState == KartMove::UMT_CHARGED) {
        driftState = KartMove::SMT_CHARGED;
    }
    return driftState;
}
...
// void PatchBoostOnUMTSpeedBoost(EGG::Effect*
//                                   boostEffect) { // have to mod loop index by 4 to get the actual index
...
//    if (umtState[effects->playerId]) {
//        boostEffect = ExpPlayerEffects::rk_purpleMT[ExpPlayerEffects::rk_purpleBoost + loopIndex % 4];
//    }
//    boostEffect->create();
//}
...
void PatchFadeBoost(EGG::Effect* boostEffect) {
    ...
    ;
    if (!effects->isBike) {
        ExpPlayerEffects::rk_purpleMT[ExpPlayerEffects::rk_purpleBoost + loopIndex % 4]->followFade();
    }
}
// kmCall(0x8069C0A4, PatchFadeBoost);

// nw4r::snd::SoundStartable::StartResult PlayExtBRSEQ(nw4r::snd::SoundStartable& startable, void* handle,
// const char* fileName, const char* labelName, bool hold) {
...
//         nw4r::snd::SoundStartable::StartResult ret =
//         PlayExtBRSEQ(reinterpret_cast<nw4r::snd::SoundStartable&>(sound), handle, seqName, labelName,
//         true); if (ret == snd::SoundStartable::START_SUCCESS) return;
//     }

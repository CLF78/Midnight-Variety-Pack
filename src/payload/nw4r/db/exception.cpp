...
void ShowGPR_(const OSContext* context) {
    ...
    for (int reg = 0; reg < 10; reg++) {
        Exception_Printf_("r%d  %08X  r%d %08X  r%d %08X\n", reg, context->gpr[reg], reg + 11,
                          context->gpr[reg + 11], reg + 22, context->gpr[reg + 22]);
    }
...
REPLACE void PrintContext_(OSError error, const OSContext* context, u32 dsisr, u32 dar) {
    ...
    Exception_Printf_("Please take a picture of the information below and send it "
                      "to " EXCEPTION_HANDLER_SUPPORT_ADDRESS ".");
    Exception_Printf_(" Use the D-Pad/Circle Pad to scroll through the data or press HOME/START to return to "
                      "the System Menu.\n");
